...
$allowedRoles = ['mtc_staff', 'mtc_judge', 'system_admin'];
if (!in_array($userRole, $allowedRoles)) {
    die("Access denied. Only authorized MTC personnel can access this module.");
}
...
// Get cases ready for escalation to RTC
$readyForEscalationStmt = $pdo->prepare("SELECT * FROM cases 
                                        WHERE stage = 'mtc_case' 
                                        AND status IN ('under_trial', 'ready_for_rtc')
                                        ORDER BY created_at ASC LIMIT 10");
...
$statusDistStmt = $pdo->prepare("SELECT status, COUNT(*) as count FROM cases 
                                WHERE stage = 'mtc_case' GROUP BY status");
...
<div class="container-fluid mt-4">
    ...
    <div class="row mb-4">
        <div class="col">
            <h1>⚖️ Municipal Trial Court (MTC) Dashboard</h1>
            <p class="text-muted">Manage trials, hearings, and escalations to RTC</p>
        </div>
    </div>
        ...
        <div class="col-md-3">
            <div class="card card-court stat-card">
                <div class="card-body">
                    ...
                    <div class="stat-label">Escalated to RTC (Month)</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="btn-group w-100" role="group">
                <a href="cases.php" class="btn btn-primary flex-fill">📋 View All Cases</a>
                <a href="#pending-section" class="btn btn-outline-info flex-fill">⏳ Pending Review (<?php echo $pendingReview; ?>)</a>
                <a href="#ready-escalation-section" class="btn btn-outline-danger flex-fill">⬆️ Ready for RTC (<?php echo count($readyForEscalation); ?>)</a>
            </div>
        </div>
    </div>
            ...
            <div class="card card-court">
                ...
                <div class="card-body">
                    <?php if (empty($recentCases)): ?>
                        <div class="alert alert-info">No cases assigned to MTC yet.</div>
                    <?php else: ?>
    ...
    <div class="row mb-4" id="pending-section">
        <div class="col-12">
            <div class="alert alert-info">
                ...
                <p class="mb-0">There are <strong><?php echo count($pendingCases); ?></strong> case(s) awaiting initial examination and assignment.</p>
            </div>
        </div>
        <div class="col-12">
            <div class="card card-court">
    ...
    <!-- Cases Ready for Escalation to RTC -->
    <?php if (!empty($readyForEscalation)): ?>
    <div class="row mb-4" id="ready-escalation-section">
        <div class="col-12">
            <div class="alert alert-warning">
                <strong>⚠️ Cases Ready for Escalation to RTC</strong>
                <p class="mb-0">There are <strong><?php echo count($readyForEscalation); ?></strong> case(s) ready to escalate to the Regional Trial Court.</p>
            </div>
        </div>
        <div class="col-12">
            <div class="card card-court">
                <div class="card-header">
                    ⬆️ Ready for RTC Escalation
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover table-sm">
